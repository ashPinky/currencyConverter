...
public class ExchangeRateDatabase {
    ...
    synchronized public void setExchangeRate(String currencyName, double rate) {
        if (CURRENCIES_MAP.get(currencyName) != null) {
            CURRENCIES_MAP.get(currencyName).setRateForOneEuro(rate);
        }
    }
}
